    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                # Store 1500 data points for one gesture
                if log_count > 1500:
                    exit()
                else:
                ...
                # # Calculate the most gesture IDs in the latest detection
...
def logging_csv(number, landmark_list, point_history_list):
    ...
    csv_path = 'model/keypoint_classifier/test_new_keypoint.csv'
...
def draw_info(image, fps, number):
    ...
    cv.putText(image, "MODE:" + mode_string, (10, 90),
                ...
                cv.LINE_AA)
    if 0 <= number <= 9:
        cv.putText(image, "LABEL_NUM:" + str(number), (10, 110),
                    cv.FONT_HERSHEY_SIMPLEX, 0.6, black, 2,
                    cv.LINE_AA)
    return image
